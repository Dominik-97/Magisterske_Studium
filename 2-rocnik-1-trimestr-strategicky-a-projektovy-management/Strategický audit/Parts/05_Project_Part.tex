...
\node [below of = c21] (c22) {Rozprodej stávajícího vozového parku};
\node [below of = c22] (c23) {Zavedení příspěvku na hromadnou, sdílenou dopravu};
\node [below of = c23] (c24) {Implementace alternativních způsobů dopravy};
\node [below of = c24] (c25) {Nákup elektro automobilů};
\node [below of = c25] (c26) {Vytvoření malého sdíleného vozového parku};

\node [below of = c3, xshift=15pt, yshift=-15pt] (c31) {Ekologická likvidace odpadů};
\node [below of = c31, yshift=-25pt] (c32) {Přizpůsobení nejvyšším enviromentálním standardům};
\node [below of = c32, yshift=-30pt] (c33) {Spotřeba energií z obnovitelných zdrojů};
